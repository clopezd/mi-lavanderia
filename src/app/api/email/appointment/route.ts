    ...
    if (type === 'created') {
      ...
      emailPromises.push(
        resend.emails.send({
          ...
          to: clientEmail,
          subject: `Pedido Confirmado con ${lawyerName} - C&C Clean Express`,
      ...
      emailPromises.push(
        resend.emails.send({
          from: EMAIL_CONFIG.from,
          to: lawyerEmail,
          subject: `Nuevo Pedido: ${clientName} - ${appointmentDate} - C&C Clean Express`,
      ...
      if (lawyerEmail !== ADMIN_EMAIL) {
        emailPromises.push(
          resend.emails.send({
            from: EMAIL_CONFIG.from,
            to: ADMIN_EMAIL,
            subject: `[Admin] Nuevo Pedido: ${clientName} con ${lawyerName} - C&C Clean Express`,
      ...
      emailPromises.push(
        resend.emails.send({
          from: EMAIL_CONFIG.from,
          to: clientEmail,
          subject: `Pedido ${statusLabels[status]} - C&C Clean Express`,
      ...
      emailPromises.push(
        resend.emails.send({
          from: EMAIL_CONFIG.from,
          to: lawyerEmail,
          subject: `Pedido ${statusLabels[status]}: ${clientName} - C&C Clean Express`,
