...
export async function generateMetadata({ params }: PageProps) {
  ...
  if (!lawyer) {
    return { title: 'Encargado no encontrado | C&C Clean Express' }
  }

  const profile = Array.isArray(lawyer.profile) ? lawyer.profile[0] : lawyer.profile

  return {
    title: `Nuevo pedido con ${profile?.full_name || 'Encargado'} | C&C Clean Express`,
    description: `Registra tu pedido con ${profile?.full_name}. Especialidad: ${lawyer.specialty}`
  }
}
...
export default async function BookLawyerPage({ params }: PageProps) {
  ...
  return (
    <PublicBookingPage
      lawyer={{
        id: lawyer.id,
        name: profile?.full_name || 'Encargado',
        email: profile?.email || '',
