// Email templates for C&C Clean Express
...
<head>
  ...
  <title>Pedido Confirmado - C&C Clean Express</title>
  <style>${baseStyles}</style>
</head>
<body>
  <div class="container">
    <div class="header">
      <h1>C&C Clean Express</h1>
      <p>Tu pedido ha sido registrado</p>
    </div>
    <div class="content">
      <p class="greeting">Hola ${data.clientName},</p>
      <p class="message">
        Tu pedido con <strong>${data.lawyerName}</strong> ha sido registrado exitosamente.
        A continuaci&oacute;n encontrar&aacute;s los detalles de tu pedido.
      </p>
      ...
      <p class="message" style="margin-top: 24px; font-size: 14px;">
        <strong>Importante:</strong> Si necesitas cancelar o reprogramar tu pedido,
        por favor hazlo con al menos 24 horas de anticipaci&oacute;n.
      </p>
    </div>
    <div class="footer">
      <p>Este correo fue enviado autom&aacute;ticamente por C&C Clean Express</p>
      <p>&copy; ${new Date().getFullYear()} C&C Clean Express</p>
    </div>
  </div>
</body>
</html>
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Nuevo Pedido - C&C Clean Express</title>
  <style>${baseStyles}</style>
</head>
<body>
  <div class="container">
    ...
    <div class="content">
      ...
      <p class="message">
        El cliente <strong>${data.clientName}</strong> ha registrado un pedido contigo.
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Nuevo Pedido en el Sistema - C&C Clean Express</title>
  <style>${baseStyles}</style>
</head>
<body>
  <div class="container">
    <div class="header" style="background: linear-gradient(135deg, #7c3aed 0%, #a855f7 100%);">
      <h1>C&C Clean Express Admin</h1>
      <p>Nuevo pedido registrado en el sistema</p>
    </div>
    <div class="content">
      <p class="greeting">Notificaci&oacute;n de Administrador</p>
      <p class="message">
        Se ha registrado un nuevo pedido en el sistema C&C Clean Express.
      </p>
    ...
    <div class="footer">
      <p>Notificaci&oacute;n autom&aacute;tica del sistema C&C Clean Express</p>
      <p>&copy; ${new Date().getFullYear()} C&C Clean Express</p>
    </div>
  </div>
</body>
...
export function appointmentStatusChangedEmail(
  ...
  const statusConfig = {
    confirmed: { label: 'Confirmado', class: 'status-confirmed', message: 'Tu pedido ha sido confirmado' },
    cancelled: { label: 'Cancelado', class: 'status-cancelled', message: 'Tu pedido ha sido cancelado' },
    completed: { label: 'Completado', class: 'status-confirmed', message: 'Tu pedido ha sido marcado como completado' },
  }
  ...
  const otherPartyLabel = data.recipientType === 'client' ? 'Encargado' : 'Cliente'
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Pedido ${config.label} - C&C Clean Express</title>
  <style>${baseStyles}</style>
</head>
<body>
  <div class="container">
